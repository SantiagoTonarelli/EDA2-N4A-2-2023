...
void problemaMochila(Elemento** elementos, int cantElementos, int capacidad){
    ...
    for (IteradorLista<Elemento*>* it = listaElementos->obtenerIterador(); it->hayElemento() && capacidad > 0; it->avanzar())
    {
        ...
        int vecesAUtilizar = min(capacidad / elem->peso, elem->cantidad);
        
        valor += vecesAUtilizar * elem->valor;
        capacidad -= vecesAUtilizar * elem->peso;
        
        cout << "Peso: " << elem->peso << " - Valor: " << elem->valor << " - Ratio: " << elem->ratio;
        cout << " - Cantidad utilizada: " << vecesAUtilizar << endl;
    }
